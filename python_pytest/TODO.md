...
- [x] 商品番号と個数を複数組、引数として受け取り、合計金額を計算する関数を書いてみよう。
- [x] 不正な商品番号
- [x] 個数が0以下
- [x] 商品リストの金額は外税なので、合計金額に消費税8％を足して、支払金額を返すようにしよう。
- [x] タバコの価格には消費税が含まれているので(内税)、消費税の計算からタバコは除かないといけない。
- [x] 消費税対象商品の合計に税率をかけないといけない(スーパーのルール)
- [x] 消費税は小数点以下切り捨て
- [x] リンゴは1個100円だが、3つ買うと280円になる。
- [x] 普通の使い方のサンプルとしてのテスト
- [x] Cartに同じ商品番号を複数回追加しても1回で追加するのと変わらない

...
class Solution:
    ...
    def minFallingPathSum(self, arr: List[List[int]]) -> int:
        ...
        def validate(picking):
            while True:
                invalid = False
                for i, v in enumerate(picking):
                    if (i > 0 and picking[i - 1] == v) or (i < len(picking) - 1 and picking[i + 1] == v):
                        invalid = True
                if not invalid:
                    return
                advance(picking)
        ...
        values = [sum([arr[row][col] for row, col in enumerate(picking)]) for picking in cols()]
